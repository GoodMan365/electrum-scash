...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-Scash"
  !define PRODUCT_WEB_SITE "https://github.com/GoodMan365/electrum-scash"
  !define PRODUCT_PUBLISHER "GoodMan365"
...
;General
  ...
  VIAddVersionKey LegalCopyright "2025-2026 ${PRODUCT_PUBLISHER}"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\electrum\gui\icons\electrum_scash.ico"
...
Section
  ...
  File "..\..\electrum\gui\icons\electrum_scash.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-scash.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-scash.exe" "" "$INSTDIR\electrum-scash.exe" 0
  ;CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-scash.exe" "--testnet" "$INSTDIR\electrum\electrum-scash.exe" 0


  ;Links bitcoin: and lightning: URIs to Electrum-Scash
  WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\electrum_scash.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\electrum\electrum-scash.exe$\" $\"%1$\""
  ...
  ;WriteRegStr HKCU "Software\Classes\lightning" "DefaultIcon" "$\"$INSTDIR\electrum_scash.ico, 0$\""
  ;WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrum-scash.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum_scash.ico"
