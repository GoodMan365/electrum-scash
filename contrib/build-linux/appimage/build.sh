...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t electrum-scash-appimage-builder-img \
    "$CONTRIB_APPIMAGE"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/electrum-scash_build/appimage/fresh_clone/electrum-scash"
...
docker run $DOCKER_RUN_FLAGS \
    --name electrum-scash-appimage-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrum-scash \
    --rm \
    --workdir /opt/electrum-scash/contrib/build-linux/appimage \
    electrum-scash-appimage-builder-img \
    ./make_appimage.sh
