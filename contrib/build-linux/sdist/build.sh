...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t electrum-scash-sdist-builder-img \
    "$CONTRIB_SDIST"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/electrum-scash_build/sdist/fresh_clone/electrum-scash"
...
docker run $DOCKER_RUN_FLAGS \
    --name electrum-scash-sdist-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrum-scash \
    --rm \
    --workdir /opt/electrum-scash/contrib/build-linux/sdist \
    --env OMIT_UNCLEAN_FILES \
    electrum-scash-sdist-builder-img \
    ./make_sdist.sh
